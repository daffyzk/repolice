...
fn main() {
    ...
    match args.depth {
        Some(d) => {
            exec_depth = d;
            println!("Not Implemented - Depth = {}, {}", d, exec_depth)
        }
        None => {}
    }
    ...
    match args.verbose {
        Some(f) => {
            exec_fetch = true;
            println!("Not Implemented - Fetch = {}, {}", f, exec_fetch)
        }
        None => {}
    }
...
fn status_message(m: String, simple: bool) -> String {
    let gb: Output = Command::new("git")
        .args(["branch", "--show-current"])
        .stdout(Stdio::piped())
        .output()
        .expect("Error!");
    let branch = String::from_utf8_lossy(&gb.stdout)
        .to_string()
        .replace("\n", "");
    match simple {
        true => {
...
fn get_repos(path: PathBuf) -> Vec<String> {
    ...
    let output: Output = Command::new("find")
        ...
        .output()
        .expect("Error!");
    let repo_results: String = String::from_utf8_lossy(&output.stdout)
        .to_string()
        .replace("/.git", "");
...
fn get_files_formatted(m: &String) -> String {
    ...
    file_list.push((
        "New".to_string(),
        get_files_list(&m, Regex::new(r"\?\? (.*)\n").unwrap()),
    ));
    file_list.push((
        "Added".to_string(),
        get_files_list(&m, Regex::new(r"A (.*)\n").unwrap()),
    ));
    file_list.push((
        "Modified".to_string(),
        get_files_list(&m, Regex::new(r"M (.*)\n").unwrap()),
    ));
    file_list.push((
        "Deleted".to_string(),
        get_files_list(&m, Regex::new(r"D (.*)\n").unwrap()),
    ));
...
fn formatted_list(list: Vec<(String, String)>) -> String {
    ...
    match e_s {
        1 => final_no_element_list = format!("{}", not_list.get(0).unwrap()).to_string(),
        2 => {
            final_no_element_list = format!(
                "{} or {}",
                not_list.get(0).unwrap(),
                not_list.get(1).unwrap()
            )
            .to_string()
        }
        3 => {
